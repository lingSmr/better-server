...
func (h *handler) servePlayer(w http.ResponseWriter, r *http.Request, relPath string) {
	...
	showHidden := r.URL.Query().Get("hidden")
	if showHidden != "" {
		backUrl += "?hidden=" + showHidden
	}

	data := PlayerData{
		Src:     "/" + filepath.ToSlash(relPath),
...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(upath, "/player/") || upath == "/player" {
		relPath := strings.TrimPrefix(upath, "/player")
...
func (h *handler) serveDownload(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range req.Paths {
		fullPath := filepath.Join(h.root, p)
